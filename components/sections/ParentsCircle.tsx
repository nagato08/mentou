...
export default function ParentsCircle({ dict }: ParentsCircleProps) {
  ...
  return (
    <section className="relative section-y border-b hairline overflow-hidden">
      {/* Background image */}
      <Image
      ...
      <div className="relative mx-auto w-full max-w-5xl px-6 md:px-12 lg:px-20 text-center" data-gsap="scale">
        ...
        <p className="mt-10 max-w-2xl mx-auto text-base md:text-lg text-bone/75 leading-relaxed">
          {t.body}
        </p>

        <ul className="mt-16 grid sm:grid-cols-3 gap-4 md:gap-6" data-gsap="stagger">
          {t.points.map((p, i) => (
            <li
              key={i}
              className="bg-ink/60 border hairline backdrop-blur-sm px-6 py-10 text-bone/90 flex flex-col items-center gap-3"
            >
